module.exports = {
    ...
    run: async (serverClient, interaction) => {
        if (!interaction.isChatInputCommand) return;
        try {
            let commandName = interaction.commandName;
            let command = global.serverClient.slashCommands.get(commandName);
            if (!command) {
                ...
                command.run(interaction, serverClient, interaction.guild, args);
            }
        } catch (error) {
            const embed = {};
            embed.author = {
